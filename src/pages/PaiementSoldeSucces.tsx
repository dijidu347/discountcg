...
import { CheckCircle, Euro, FileCheck, ArrowRight } from "lucide-react";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { formatPrice } from "@/lib/utils";

export default function PaiementSoldeSucces() {
  ...
  const newBalance = parseFloat(searchParams.get("balance") || "0");
        ...
        <CardContent className="pt-8 pb-6 px-6 text-center space-y-6">
          ...
          >
            ...
            <p className="text-muted-foreground">
              Votre démarche a été payée avec votre solde
            </p>
          </motion.div>
          ...
          >
            <p className="text-sm text-blue-700 dark:text-blue-300">
              ✅ Votre démarche est maintenant en cours de traitement
            </p>
          </motion.div>
          ...
          >
            ...
            <Button 
              onClick={() => navigate(`/demarche/${demarcheId}`)}
              className="w-full"
            >
              Voir ma démarche
